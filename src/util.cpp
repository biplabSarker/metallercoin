...
const char * const BITCOIN_CONF_FILENAME = "metallercoin.conf";
const char * const BITCOIN_PID_FILENAME = "metallercoin.pid";
...
#else
    const char* pszModule = "metallercoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Metallercoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Metallercoin";
#else
    // Unix
    return pathRet / ".metallercoin";
#endif
#endif
}
